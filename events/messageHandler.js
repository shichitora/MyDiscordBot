        ...
        try {
          ...
          if (response.headers.get('content-type')?.startsWith('image/')) {
            if (Number(response.headers.get('content-length')) > 10 * 1024 * 1024) {
              console.log(`Skipping large image: ${attachment.url} (${response.headers.get('content-length')} bytes)`);
              continue;
            }
            ...
            for (const [rule, enabled] of Object.entries(guildSettings.antiTroll.rules)) {
              ...
              if (patterns[rule] && text.match(patterns[rule])) {
                violation = rule;
                console.log(`Violation in attachment image: ${rule} - ${attachment.url} - Text: ${text.slice(0, 50)}...`);
                break;
              }
            }
          }
      ...
      if (videoExts.includes(ext)) {
        ...
        try {
          ...
          if (response.headers.get('content-type')?.startsWith('video/')) {
            if (Number(response.headers.get('content-length')) > 10 * 1024 * 1024) {
              console.log(`Skipping large video: ${attachment.url} (${response.headers.get('content-length')} bytes)`);
              continue;
            }
            const buffer = await response.buffer();
            tempFile = path.join(process.env.HOME, `temp_${Date.now()}${ext}`);
            framesDir = path.join(process.env.HOME, `frames_${Date.now()}`);
            ...
            for (const frame of frameFiles) {
              ...
              for (const [rule, enabled] of Object.entries(guildSettings.antiTroll.rules)) {
                ...
                if (patterns[rule] && text.match(patterns[rule])) {
                  violation = rule;
                  console.log(`Violation in attachment video frame: ${rule} - ${attachment.url} - Text: ${text.slice(0, 50)}...`);
                  break;
                }
              }
              if (violation) break;
      ...
      if (textExts.includes(ext)) {
        try {
          ...
          for (const [rule, enabled] of Object.entries(guildSettings.antiTroll.rules)) {
            ...
            if (patterns[rule] && text.match(patterns[rule])) {
              violation = rule;
              console.log(`Violation in attachment text file: ${rule} - ${filename} - Text: ${text.slice(0, 50)}...`);
              break;
            }
          }
        } catch (err) {
      ...
      for (const [rule, enabled] of Object.entries(guildSettings.antiTroll.rules)) {
        ...
        if (patterns[rule]) {
          if (
            ...
            violation = rule;
            console.log(`Violation in embed text: ${rule} - ${embed.url || embed.title}`);
            break;
          }
          if (embed.fields?.length > 0) {
            for (const field of embed.fields) {
              if (field.name?.match(patterns[rule]) || field.value?.match(patterns[rule])) {
                violation = rule;
                console.log(`Violation in embed field: ${rule} - ${field.name}`);
                break;
              }
            }
          }
          if (embed.author && (embed.author.name?.match(patterns[rule]) || embed.author.url?.match(patterns[rule]))) {
            violation = rule;
            console.log(`Violation in embed author: ${rule} - ${embed.author.name}`);
            break;
          }
        }
        if (violation) break;
      ...
      if (!violation && (embed.thumbnail?.url || embed.image?.url)) {
        ...
        if (imageExts.includes(ext)) {
          try {
            ...
            if (response.headers.get('content-type')?.startsWith('image/')) {
              if (Number(response.headers.get('content-length')) > 10 * 1024 * 1024) {
                console.log(`Skipping large image: ${imgUrl} (${response.headers.get('content-length')} bytes)`);
                continue;
              }
              const buffer = await response.buffer();
              const { data: { text } } = await Tesseract.recognize(buffer, 'eng', { logger: (m) => console.log(m) });
              for (const [rule, enabled] of Object.entries(guildSettings.antiTroll.rules)) {
                ...
                if (patterns[rule] && text.match(patterns[rule])) {
                  violation = rule;
                  console.log(`Violation in embed image: ${rule} - ${imgUrl} - Text: ${text.slice(0, 50)}...`);
                  break;
                }
              }
            }
      ...
      if (!violation && embed.video?.url) {
        ...
        if (videoExts.includes(ext)) {
          try {
            ...
            if (response.headers.get('content-type')?.startsWith('video/')) {
              if (Number(response.headers.get('content-length')) > 10 * 1024 * 1024) {
                console.log(`Skipping large video: ${embed.video.url} (${response.headers.get('content-length')} bytes)`);
                continue;
              }
              const buffer = await response.buffer();
              tempFile = path.join(process.env.HOME, `temp_embed_${Date.now()}${ext || '.mp4'}`);
              framesDir = path.join(process.env.HOME, `frames_embed_${Date.now()}`);
              await fs.writeFile(tempFile, buffer);
              await fs.mkdir(framesDir);
              ...
              for (const frame of frameFiles) {
                ...
                for (const [rule, enabled] of Object.entries(guildSettings.antiTroll.rules)) {
                  ...
                  if (patterns[rule] && text.match(patterns[rule])) {
                    violation = rule;
                    console.log(`Violation in embed video frame: ${rule} - ${embed.video.url} - Text: ${text.slice(0, 50)}...`);
                    break;
                  }
                }
                if (violation) break;
    ...
    if (snapshotContent) {
      for (const [rule, enabled] of Object.entries(guildSettings.antiTroll.rules)) {
        ...
        if (patterns[rule]?.test(snapshotContent)) {
          violation = rule;
          console.log(`Violation in snapshot content: ${rule} - ${snapshotContent.slice(0, 50)}...`);
          break;
        }
      }
    }
        ...
        for (const [rule, enabled] of Object.entries(guildSettings.antiTroll.rules)) {
          ...
          if (patterns[rule]) {
            if (
              ...
              violation = rule;
              console.log(`Violation in snapshot embed text: ${rule} - ${embed.url || embed.title}`);
              break;
            }
            if (embed.fields?.length > 0) {
              for (const field of embed.fields) {
                if (field.name?.match(patterns[rule]) || field.value?.match(patterns[rule])) {
                  violation = rule;
                  console.log(`Violation in snapshot embed field: ${rule} - ${field.name}`);
                  break;
                }
              }
            }
            if (embed.author && (embed.author.name?.match(patterns[rule]) || embed.author.url?.match(patterns[rule]))) {
              violation = rule;
              console.log(`Violation in snapshot embed author: ${rule} - ${embed.author.name}`);
              break;
            }
          }
          if (violation) break;
            ...
            try {
              ...
              if (response.headers.get('content-type')?.startsWith('image/')) {
                ...
                for (const [rule, enabled] of Object.entries(guildSettings.antiTroll.rules)) {
                  ...
                  if (patterns[rule] && text.match(patterns[rule])) {
                    violation = rule;
                    console.log(`Violation in snapshot embed image: ${rule} - ${imgUrl} - Text: ${text.slice(0, 50)}...`);
                    break;
                  }
                }
              }
          ...
          if (videoExts.includes(ext)) {
            try {
              ...
              if (response.headers.get('content-type')?.startsWith('video/')) {
                ...
                const buffer = await response.buffer();
                tempFile = path.join(process.env.HOME, `temp_snap_${Date.now()}${ext || '.mp4'}`);
                framesDir = path.join(process.env.HOME, `frames_snap_${Date.now()}`);
                await fs.writeFile(tempFile, buffer);
                await fs.mkdir(framesDir);
                await new Promise((resolve, reject) => {
                  ffmpeg(tempFile)
                ...
                for (const frame of frameFiles) {
                  ...
                  for (const [rule, enabled] of Object.entries(guildSettings.antiTroll.rules)) {
                    ...
                    if (patterns[rule] && text.match(patterns[rule])) {
                      violation = rule;
                      console.log(`Violation in snapshot embed video frame: ${rule} - ${embed.video.url} - Text: ${text.slice(0, 50)}...`);
                      break;
                    }
                  }
                  if (violation) break;
    ...
    if (!violation && snapshot.attachments?.size > 0) {
      for (const attachment of snapshot.attachments.values()) {
        ...
        for (const [rule, enabled] of Object.entries(guildSettings.antiTroll.rules)) {
          ...
          if (patterns[rule] && filename.match(patterns[rule])) {
            violation = rule;
            console.log(`Violation in snapshot attachment filename: ${rule} - ${filename}`);
            break;
          }
        }
        if (violation) break;
          ...
          try {
            ...
            if (response.headers.get('content-type')?.startsWith('image/')) {
              ...
              for (const [rule, enabled] of Object.entries(guildSettings.antiTroll.rules)) {
                ...
                if (patterns[rule] && text.match(patterns[rule])) {
                  violation = rule;
                  console.log(`Violation in snapshot attachment image: ${rule} - ${attachment.url} - Text: ${text.slice(0, 50)}...`);
                  break;
                }
              }
            }
        ...
        if (videoExts.includes(ext)) {
          ...
          try {
            ...
            if (response.headers.get('content-type')?.startsWith('video/')) {
              ...
              const buffer = await response.buffer();
              tempFile = path.join(process.env.HOME, `temp_snap_${Date.now()}${ext}`);
              framesDir = path.join(process.env.HOME, `frames_snap_${Date.now()}`);
              await fs.writeFile(tempFile, buffer);
              await fs.mkdir(framesDir);
              ...
              for (const frame of frameFiles) {
                ...
                for (const [rule, enabled] of Object.entries(guildSettings.antiTroll.rules)) {
                  ...
                  if (patterns[rule] && text.match(patterns[rule])) {
                    violation = rule;
                    console.log(`Violation in snapshot attachment video frame: ${rule} - ${attachment.url} - Text: ${text.slice(0, 50)}...`);
                    break;
                  }
                }
                if (violation) break;
        ...
        if (textExts.includes(ext)) {
          try {
            ...
            for (const [rule, enabled] of Object.entries(guildSettings.antiTroll.rules)) {
              ...
              if (patterns[rule] && text.match(patterns[rule])) {
                violation = rule;
                console.log(`Violation in snapshot attachment text file: ${rule} - ${filename} - Text: ${text.slice(0, 50)}...`);
                break;
              }
            }
          } catch (err) {
